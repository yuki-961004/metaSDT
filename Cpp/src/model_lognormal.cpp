...
T ModelLognormal<T>::area(
    ...
    if (response == 1) {
        ...
        if (std::abs(lower - c_resp) < static_cast<T>(1e-9)) {
            ...
            const T dist = lower - c_resp;
            const T crit_log = (dist > static_cast<T>(1e-6))
                ? std::log(dist)
                : static_cast<T>(-10.0);
            p_lower = prob_high_conf_lognormal(mu_s2, crit_log, sigma_meta);
        }
        ...
        if (upper >= inf_thresh) {
            ...
            const T dist = upper - c_resp;
            const T crit_log = (dist > static_cast<T>(1e-6))
                ? std::log(dist)
                : static_cast<T>(-10.0);
            p_upper = prob_high_conf_lognormal(mu_s2, crit_log, sigma_meta);
        }
        ...
        if (std::abs(upper - c_resp) < static_cast<T>(1e-9)) {
            ...
            const T dist = c_resp - upper;
            const T crit_log = (dist > static_cast<T>(1e-6))
                ? std::log(dist)
                : static_cast<T>(-10.0);
            p_upper_mirror = prob_high_conf_lognormal(mu_s1, crit_log, sigma_meta);
        }
        ...
        if (lower <= -inf_thresh) {
            ...
            const T dist = c_resp - lower;
            const T crit_log = (dist > static_cast<T>(1e-6))
                ? std::log(dist)
                : static_cast<T>(-10.0);
            p_lower_mirror = prob_high_conf_lognormal(mu_s1, crit_log, sigma_meta);
        }
