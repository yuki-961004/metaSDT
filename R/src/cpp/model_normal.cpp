...
ModelNormal<T>::ModelNormal(
    ...
    for (std::size_t i = 0; i < d_vec.size(); ++i) {
        if (d_vec[i] <= static_cast<T>(0.0)) {
            throw std::invalid_argument(
                "ModelNormal Initialization Error: 'd' values must be positive."
            );
        }
    }
    ...
    if (sigma_meta <= static_cast<T>(0.0)) {
        throw std::invalid_argument(
            "ModelNormal Initialization Error: 'sigma_meta' must be strictly "
            "positive."
        );
    }
    ...
    if (sd_noise != static_cast<T>(1.0) || sd_signal != static_cast<T>(1.0)) {
        throw std::invalid_argument(
            "ModelNormal Initialization Error: ModelNormal requires equal "
...
T ModelNormal<T>::area(
    ...
    if (response == 1) {
        ...
        if (std::abs(lower - c_resp) < static_cast<T>(1e-9)) {
            ...
            p_lower = prob_high_conf_normal(mu_stim, c_resp, lower, sigma_meta);
        }
        ...
        if (upper >= inf_thresh) {
            ...
            p_upper = prob_high_conf_normal(mu_stim, c_resp, upper, sigma_meta);
        }
        ...
        if (std::abs(upper - c_resp) < static_cast<T>(1e-9)) {
            ...
            p_upper_mirror = prob_high_conf_normal(-mu_stim, -c_resp, -upper, sigma_meta);
        }
        ...
        if (lower <= -inf_thresh) {
            ...
            p_lower_mirror = prob_high_conf_normal(-mu_stim, -c_resp, -lower, sigma_meta);
        }
...
ModelNormal<T>::compute_probabilities() const {
...
template <typename T>
MatrixProb<T> ModelNormal<T>::area() const {
    MatrixProb<T> res;
