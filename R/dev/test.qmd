...
indices_sub29 <- test$subjects$`29`$raw

# 2. 使用这串行号，从原始的、完整的 `data` 数据框中进行切片，得到真实的 dataframe
df_sub29 <- data[indices_sub29, ]
...
matrix_mult(freq_mat, prob_mat, final_params)
```