...
# 剥离常数，得到 C++ 引擎实际使用的核心核函数：(a-1)*log(x) + (b-1)*log(1-x)
log_prior_c_conf_cpp <- log_prior_c_conf_full - beta_constant

# 3. 汇总对比
total_prior_full <- log_prior_d + log_prior_c_conf_full
total_prior_cpp <- log_prior_d + log_prior_c_conf_cpp

cat("R 原生 dnorm + dbeta 计算结果 (含常数):", total_prior_full, "\n")
cat("C++ 优化核函数计算结果 (剥离常数):", total_prior_cpp, "\n")
