...
pybind11::dict py_info_data(
    ...
    pybind11::dict out;
    // pybind11::cast 自动处理 unordered_map 转换为 dict
    out["colnames"] = pybind11::cast(res.colnames); 
    out["subjects"] = r_subjects;
    return out;
}

PYBIND11_MODULE(_help_info_data, m) {
    // 注册 Python 类, 使得 Python 端可以通过(.)访问
    ...
    m.def(
        "info_data",
        &py_info_data,
        "Intelligently scan the dataset and extract subject-level "
        "information.",
        pybind11::arg("df"),
