...
ModelLognormal<T>::ModelLognormal(
    ...
    if (it_d == params.end() || it_d->second.empty()) {
        throw std::invalid_argument(
            "ModelLognormal Initialization Error: Missing parameter 'd'."
        );
    }
    d_vec = it_d->second;
    ...
    for (std::size_t i = 0; i < d_vec.size(); ++i) {
        if (d_vec[i] <= static_cast<T>(0.0)) {
            throw std::invalid_argument(
                "ModelLognormal Initialization Error: 'd' values must be "
                "positive."
            );
        }
    }
    ...
    if (sigma_meta <= static_cast<T>(0.0)) {
        throw std::invalid_argument(
            "ModelLognormal Initialization Error: 'sigma_meta' must be "
            "strictly positive."
        );
    }
    ...
    if (sd_noise != static_cast<T>(1.0) || sd_signal != static_cast<T>(1.0)) {
        throw std::invalid_argument(
            "ModelLognormal Initialization Error: ModelLognormal requires equal "
...
template <typename T>
T prob_high_conf_lognormal(
    const T& mu1,
    const T& conf_crit,
    const T& sigma_meta
) {
    const T a = static_cast<T>(1e-6);
    const T b = std::max(static_cast<T>(1.0), mu1) + static_cast<T>(7.0);
    if (b <= a) {
        return static_cast<T>(0.0);
    }

    auto integrand = [&](const T& x) -> T {
        const T log_x = std::log(x);
...
ModelLognormal<T>::compute_probabilities() const {
    ...
    for (std::size_t d_idx = 0; d_idx < n_diffs; ++d_idx) {
        ...
        for (std::size_t stim = 0; stim < 2; ++stim) {
            ...
            for (std::size_t k = 0; k < n_ratings - 1; ++k) {
                const T dist = std::abs(c_resp - criteria[k]);
                const T crit_log = (dist > static_cast<T>(1e-6))
                    ? std::log(dist)
                    : static_cast<T>(-10.0);
                p_neg[k] = prob_high_conf_lognormal(
                    mu_s1,
                    crit_log,
                    sigma_meta
                );
            ...
            for (std::size_t k = 0; k < n_ratings - 1; ++k) {
                const T dist = std::abs(criteria[mid_idx + 1 + k] - c_resp);
                const T crit_log = (dist > static_cast<T>(1e-6))
                    ? std::log(dist)
                    : static_cast<T>(-10.0);
                p_pos[k] = prob_high_conf_lognormal(
                    mu_s2,
                    crit_log,
                    sigma_meta
                );
...
T ModelLognormal<T>::area(
    ...
    if (response == 1) {
        ...
        const T log_lower = std::log(std::abs(lower - c_resp));
        const T log_upper = std::log(std::abs(upper - c_resp));
        const T p_lower = prob_high_conf_lognormal(
            mu_s,
            log_lower,
            sigma_meta
        );
        const T p_upper = prob_high_conf_lognormal(
            mu_s,
            log_upper,
            sigma_meta
        );
        ...
        const T mu_s = -mu_stim + c_resp;
        const T log_lower = std::log(std::abs(c_resp - lower));
        const T log_upper = std::log(std::abs(c_resp - upper));
        const T p_lower = prob_high_conf_lognormal(
            mu_s,
            log_lower,
            sigma_meta
...
template <typename T>
MatrixProb<T> ModelLognormal<T>::area() const {
    MatrixProb<T> res;
