...
std::vector<SubjectFitTask> build_fit_tasks(
    ...
    // 核心防御：只有当模型实际包含 c_conf 时，才读取数据的 conf 列
    if (has_conf_params) {
...
double nll(unsigned n, const double* x, double* grad, void* f_data) {
    try {
        ...
        if (task->model == "sdt") {
            // 指定泛型为 double，无缝对接 NLOPT
            ModelSDT<double> model(current_params);
        ...
        // 致命防御：绝不允许 C++ 异常跨越边界抛给纯 C 语言编写的 NLOPT！
        // 否则会导致多线程崩溃或优化器死锁。出现非法状态时，返回正无穷大，迫使优化器回退重试。
        return 1e10;
    }
}