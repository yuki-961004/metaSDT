...
def matrix_mult(freq_mat, prob_mat, params):
    ...
    p_mat = prob_mat.values.tolist() if isinstance(prob_mat, pandas.DataFrame) else prob_mat
...
def loss_function(mult_mat, freq_mat, params):
    """
    Calculate Model Loss indicators including Negative Log-Likelihood, AIC, and BIC.
    """
    # 安全地将 DataFrame 转为 C++ 能够接收的嵌套列表
    m_mat = mult_mat.values.tolist() if isinstance(mult_mat, pandas.DataFrame) else mult_mat
    f_mat = freq_mat.values.tolist() if isinstance(freq_mat, pandas.DataFrame) else freq_mat
        
    return _core_loss_function.loss_function(m_mat, f_mat, params)
